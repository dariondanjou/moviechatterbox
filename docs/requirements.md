...
**Version:** 1.4 | **Date:** July 4, 2026 **Owner:** Darion D'Anjou, Founder/Operator — a Darion D'Anjou project **Status:** Approved decisions from monetization & architecture planning session, plus existing-site audit, competitor feature absorption, Chatterbox naming, AI personalization, data-use policy, and future-state vision (§8–13)
...
- Launch focused on a single fandom vertical (recommended: horror) before widening. Launch is **audio-first**: the first public release ships working live-audio Chatterboxes.  
...
- FR-2.1.6: **Audio-first mandate:** Chatterboxes launch with live audio from v1.0. Live audio is the product's core identity and ships in the first store release; text chat within rooms is a companion and degradation channel (FR-2.1.5), never a substitute launch medium.
