...
const movieRouter = router({
  ...
  search: publicProcedure
    ...
    .query(async ({ input }) => {
      const db = await getDb();
      if (!db || !input.q.trim()) return { movies: [], persons: [] };
  ...
  upsert: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      const existing = await db.select().from(ratings)
        .where(and(eq(ratings.userId, ctx.user.id), eq(ratings.movieId, input.movieId)))
        .limit(1);
      if (existing.length > 0) {
        await db.update(ratings).set({ rating: input.rating }).where(eq(ratings.id, existing[0].id));
      } else {
        await db.insert(ratings).values({ userId: ctx.user.id, movieId: input.movieId, rating: input.rating });
      }
      // Update movie avg
  ...
  create: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      await db.insert(reviews).values({
        userId: ctx.user.id, movieId: input.movieId,
  ...
  toggle: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      const [existing] = await db.select().from(watchlist)
        .where(and(eq(watchlist.userId, ctx.user.id), eq(watchlist.movieId, input.movieId)))
        .limit(1);
      if (existing) {
        await db.delete(watchlist).where(eq(watchlist.id, existing.id));
        return { added: false };
      } else {
        await db.insert(watchlist).values({ userId: ctx.user.id, movieId: input.movieId });
        return { added: true };
      }
    }),
  ...
  create: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      const [result] = await db.insert(threads).values({
        movieId: input.movieId, userId: ctx.user.id, title: input.title, body: input.body,
      });
  ...
  reply: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      await db.insert(threadReplies).values({
        threadId: input.threadId, userId: ctx.user.id,
  ...
  join: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      const existing = await db.select().from(roomParticipants)
        .where(and(eq(roomParticipants.roomId, input.roomId), eq(roomParticipants.userId, ctx.user.id), isNull(roomParticipants.leftAt)))
        .limit(1);
      if (existing.length === 0) {
        await db.insert(roomParticipants).values({
          roomId: input.roomId, userId: ctx.user.id, role: "listener",
        });
  ...
  leave: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      await db.update(roomParticipants)
        .set({ leftAt: new Date() })
        .where(and(eq(roomParticipants.roomId, input.roomId), eq(roomParticipants.userId, ctx.user.id)));
